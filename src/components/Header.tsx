...
import { Train, Flame, Shield, Crosshair, Gift, Database, MapPin, Menu, X, Search } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50 bg-[#070a0f]/90 backdrop-blur-md border-b border-amber-500/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center space-x-1 font-mono text-xs text-slate-300">
          <Link href="/fuel-calculator" className="px-3 py-2 rounded-lg hover:bg-slate-800/80 hover:text-amber-400 transition-colors flex items-center gap-1.5">
            <Flame className="w-3.5 h-3.5 text-amber-400" /> Fuel Calc
          </Link>
          <Link href="/weapons-tier-list" className="px-3 py-2 rounded-lg hover:bg-slate-800/80 hover:text-red-400 transition-colors flex items-center gap-1.5">
            <Crosshair className="w-3.5 h-3.5 text-red-400" /> Weapons Tier
          </Link>
          <Link href="/codes" className="px-3 py-2 rounded-lg hover:bg-slate-800/80 hover:text-emerald-400 transition-colors flex items-center gap-1.5">
            <Gift className="w-3.5 h-3.5 text-emerald-400" /> Active Codes
          </Link>
          <Link href="/train-upgrades" className="px-3 py-2 rounded-lg hover:bg-slate-800/80 hover:text-cyan-400 transition-colors flex items-center gap-1.5">
            <Shield className="w-3.5 h-3.5 text-cyan-400" /> Train Upgrades
          </Link>
          <Link href="/zombies-guide" className="px-3 py-2 rounded-lg hover:bg-slate-800/80 hover:text-purple-400 transition-colors flex items-center gap-1.5">
            <Database className="w-3.5 h-3.5 text-purple-400" /> Zombies
          </Link>
          <Link href="/routes-map" className="px-3 py-2 rounded-lg hover:bg-slate-800/80 hover:text-amber-400 transition-colors flex items-center gap-1.5">
            <MapPin className="w-3.5 h-3.5 text-amber-400" /> Stations Map
          </Link>
        </nav>
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="lg:hidden bg-[#070a0f] border-b border-slate-800 px-4 pt-2 pb-4 space-y-1 font-mono text-sm">
